...
Run() {
	...
	NUM_M=$((NUM / 2))
	NUM_F=$((NUM / 2))
	[ 1 -eq $((NUM % 2)) ] && { [ 0 -eq $((RANDOM % 2)) ] && NUM_M=$((NUM_M + 1)) || NUM_F=$((NUM_M + 1)); }
	RESULT="$(Execute "$(JoinNames 'm' $NUM_M)" "$(JoinNames 'f' $NUM_F)")"
#	[ 0 -eq $((NUM % 2)) ] && {
#		RESULT="$(Execute "$(JoinNames 'm' $((NUM / 2)))" "$(JoinNames 'f' $((NUM / 2)))")"
#	} || {
#		RESULT="$(Execute "$(JoinNames 'm' $((NUM / 2)))" "$(JoinNames 'f' $((NUM / 2)))" "$(JoinNames '*' 1)")"
#	}
